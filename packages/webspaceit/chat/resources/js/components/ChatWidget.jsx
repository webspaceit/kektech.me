...
export default function ChatWidget({ accentColor = 'emerald', title = 'Live Chat' }) {
        ...
        if (res.ok) {
            ...
            if (data.length > prevMsgCount.current) {
                ...
                if (newMsgs.some(m => !m.is_guest)) {
                    ...
                    showDesktopNotification(title, adminMsg?.message || 'You have a new message');
                }
            }
            prevMsgCount.current = data.length;
            setMessages(data);
    ...
    const colorClasses = {
        emerald: { bg: 'bg-emerald-600', hover: 'hover:bg-emerald-500', text: 'text-emerald-200', border: 'border-emerald-600', light: 'bg-emerald-600/20', userBubble: 'bg-emerald-600 text-white' },
        blue: { bg: 'bg-blue-600', hover: 'hover:bg-blue-500', text: 'text-blue-200', border: 'border-blue-600', light: 'bg-blue-600/20', userBubble: 'bg-blue-600 text-white' },
        purple: { bg: 'bg-purple-600', hover: 'hover:bg-purple-500', text: 'text-purple-200', border: 'border-purple-600', light: 'bg-purple-600/20', userBubble: 'bg-purple-600 text-white' },
        red: { bg: 'bg-red-600', hover: 'hover:bg-red-500', text: 'text-red-200', border: 'border-red-600', light: 'bg-red-600/20', userBubble: 'bg-red-600 text-white' },
        orange: { bg: 'bg-orange-600', hover: 'hover:bg-orange-500', text: 'text-orange-200', border: 'border-orange-600', light: 'bg-orange-600/20', userBubble: 'bg-orange-600 text-white' },
    };
            ...
            {open ? (
                <div className="w-80 h-96 bg-white dark:bg-gray-800 rounded-xl shadow-2xl border border-gray-200 dark:border-gray-700 flex flex-col overflow-hidden">
                    <div className={`${c.bg} text-white px-4 py-3 flex items-center justify-between`}>
                        ...
                        <button onClick={() => setOpen(false)} className={`${c.hover} rounded p-1 transition`}>
                            <X className="w-4 h-4" />
                        </button>
                    </div>
                    ...
                    {!started ? (
                        <form onSubmit={startChat} className="p-4 flex-1 flex flex-col gap-3">
                            ...
                            <input type="text" placeholder="Your name" value={name} onChange={e => setName(e.target.value)} required
                                className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md text-sm dark:bg-gray-700 dark:text-white text-black" />
                            <input type="email" placeholder="Your email" value={email} onChange={e => setEmail(e.target.value)} required
                                className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md text-sm dark:bg-gray-700 dark:text-white text-black" />
                            <button type="submit" className={`w-full py-2 ${c.bg} text-white text-sm font-medium rounded-md ${c.hover} transition`}>Start Chat</button>
                        </form>
                    ) : (
                        <>
                                ...
                                {messages.length === 0 ? (
                                    ...
                                    messages.map(msg => (
                                        <div key={msg.id} className={`flex ${msg.is_guest ? 'justify-end' : 'justify-start'}`}>
                                            <div className={`max-w-[80%] rounded-lg px-3 py-2 text-xs ${msg.is_guest ? c.userBubble : 'bg-gray-100 dark:bg-gray-700 text-gray-900 dark:text-white'}`}>
                                                <p>{msg.message}</p>
                                                <p className={`mt-1 ${msg.is_guest ? c.text : 'text-gray-500'}`}>{new Date(msg.created_at).toLocaleTimeString()}</p>
                                            </div>
                                        </div>
                                    ))
                                )}
                            ...
                            <form onSubmit={sendMessage} className="p-2 border-t border-gray-200 dark:border-gray-700 flex gap-1">
                                <input type="text" value={newMessage} onChange={e => setNewMessage(e.target.value)} placeholder="Type a message..."
                                    className="flex-1 px-2 py-1.5 border border-gray-300 dark:border-gray-600 rounded text-xs dark:bg-gray-700 dark:text-white text-black" />
                                <button type="submit" className={`px-2 py-1.5 ${c.bg} text-white rounded ${c.hover} transition`}>
                ...
                <button onClick={() => { setOpen(true); requestNotifPermission(); }}
                    className={`w-14 h-14 ${c.bg} ${c.hover} text-white rounded-full shadow-lg flex items-center justify-center transition-all hover:scale-110`}>
